...
graduate_data1 = pd.read_excel('新冠疫情对备考研究生心理影响分析_517_517.xlsx', sheet_name='最终数据', index_col='序号', header=0)
graduate_data = graduate_data1.iloc[0:400,:]
print(graduate_data)
...
def calculate_p_in_stick_and_give(gender='男生',motion='不焦虑',antitude='积极',fanical='没损失',recruit='扩大招生',prepare_exam='备考没有更加困难'):
...
gender_ = {0:'男生',1:'女生'}
motion_ = {0:'焦虑',1:'不焦虑'}
antitude_ = {0:'消极',1:'积极',2:'二者之间'}
fanical_ = {0:'有损失',1:'没损失'}
recruit_ = {0:'扩大招生',1:'没扩大招生'}
prepare_exam_ = {0:'备考更加困难',1:'备考没有更加困难'}
if __name__ == '__main__':

    p_stick, p_giveup = calculate_p_in_stick_and_give('男生','焦虑','消极','有损失','没扩大招生','备考更加困难')
    graduate_data_test = graduate_data1.iloc[400:,:]
    j = 0
    z = 0
    for i in range(len(graduate_data_test)):
        gender_1 = gender_[graduate_data_test.iloc[i, 0]]
        print(graduate_data_test.iloc[i, 0])
        print(gender_1)
        motion_1 = motion_[graduate_data_test.iloc[i, 1]]
        print(graduate_data_test.iloc[i, 1])
        print(motion_1)
        antitude_1 = antitude_[graduate_data_test.iloc[i, 2]]
        print(graduate_data_test.iloc[i, 2])
        print(antitude_1)
        fanical_1 = fanical_[graduate_data_test.iloc[i, 3]]
        print(graduate_data_test.iloc[i, 3])
        print(fanical_1)
        recruit_1 = recruit_[graduate_data_test.iloc[i, 4]]
        print(graduate_data_test.iloc[i, 4])
        print(recruit_1)
        prepare_exam_1 = prepare_exam_[graduate_data_test.iloc[i, 5]]
        print(graduate_data_test.iloc[i, 5])
        print(prepare_exam_1)
            p_stick, p_giveup = calculate_p_in_stick_and_give(gender=gender_1,motion=motion_1,antitude=antitude_1,fanical=fanical_1,recruit=recruit_1,prepare_exam=prepare_exam_1)
        ...
        if p_stick > p_giveup:
            ...
            result = 1
        if result == graduate_data_test.iloc[i, 6]:
            z = z + 1
    accuracy_test = z / len(graduate_data_test)
    print(f'该测试样本的数据集准确率为{accuracy_test}')
